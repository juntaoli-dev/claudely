...
export const getUserInfo = (): UserProfile | null => {
  ...
  const storedUserInfo = localStorage.getItem('claudely_user');
  if (storedUserInfo) {
    try {
      return JSON.parse(storedUserInfo);
    } catch (error) {
      console.error('Failed to parse user info:', error);
      localStorage.removeItem('claudely_user');
    }
  }
  return null;
};
...
export const setUserInfo = (userInfo: UserProfile | null, skipEvents: boolean = false) => {
  ...
  if (userInfo) {
    localStorage.setItem('claudely_user', JSON.stringify(userInfo));
  } else {
    localStorage.removeItem('claudely_user');
  }
